...
import { realisations, categories, regions } from "@/data/realisations";
import ScrollReveal from "@/components/ScrollReveal";

export default function RealisationsClient() {
  const [cat, setCat] = useState("Tous");
  const [reg, setReg] = useState("Toutes");

  const filtered = realisations.filter(
    (r) => (cat === "Tous" || r.category === cat) && (reg === "Toutes" || r.region === reg)
  );
    ...
    <>
      ...
      <section className="relative bg-navy-950 text-white min-h-[60vh] flex items-center overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10 py-32 pt-40">
          <div className="max-w-2xl">
            <span className="text-orange-400 text-xs font-bold uppercase tracking-[0.3em] mb-4 block animate-fade-in-up">Portfolio</span>
            <div className="accent-line mb-6 animate-fade-in-up" style={{ animationDelay: "0.1s" }} />
            <h1 className="text-5xl md:text-7xl font-black tracking-tight mb-5 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>Nos réalisations</h1>
            <p className="text-white/50 text-lg max-w-xl leading-relaxed animate-fade-in-up" style={{ animationDelay: "0.3s" }}>
              Portfolio de projets complétés à travers le Québec.
            </p>
          </div>
        </div>
      ...
      <section className="bg-white py-24 md:py-32 relative overflow-hidden">
        <div className="container mx-auto px-6 max-w-6xl relative z-10">
          ...
          <div className="bg-navy-50 p-6 mb-12 relative overflow-hidden">
            ...
            <div className="flex flex-wrap gap-6">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs font-bold text-navy-400 uppercase tracking-wider mr-2">Catégorie</span>
                {categories.map((c) => (
                  <button
                    key={c}
                    onClick={() => setCat(c)}
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-navy-400 uppercase tracking-wider">Région</span>
                <select
                  value={reg}
          ...
          {filtered.length === 0 ? (
            <p className="text-center text-navy-400 py-16 text-sm">Aucune réalisation trouvée pour ces critères.</p>
          ) : (
